...
namespace AnimationBehaviors
{
    public class WalkBehavior : StateMachineBehaviour
    {
        ...
        void HandleLayers(Animator animator)
        {
            ...
            {
                ActivateLayer("WalkLayer", animator);
                AnimateMovement(animator);
            }
        }
    
        private void AnimateMovement(Animator animator)
        {
            //Sets the animation parameter so character animation walks in the correct direction
            animator.SetFloat(Vx, _movementController.MoveDirection.x);
            animator.SetFloat(Vy, _movementController.MoveDirection.y);
        }
